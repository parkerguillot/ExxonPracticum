...
urls = 'https://www.energycentral.com/posts'


class EnCentralSpider(scrapy.Spider):
    name = 'encentral'
    page_number = 1
    ...
    def parse_front(self, response):
        article_links = response.xpath('.//span[@class = "article-title"]/a/href')
        print(article_links)
        ...
        next_page = 'https://www.energycentral.com/posts#keywords%3D%26sort_type%3Ddate_newest%26page%3D' + str(EnCentralSpider.page_number)
        if EnCentralSpider.page_number <= 400:
            EnCentralSpider.page_number += 1
...
# response.xpath('//div[(@class="field field-name-field-twitter-id field-type-text field-label-hidden")]/a/@href]')
        article_date = response.xpath('//span[@class = "article-verb_date"]/text()').re(r"\w+\s\d{1,2},\s\d{4}")
        article_title = response.xpath('//h1//text()').extract_first()
        author = response.xpath('//span[@class = "realname"]//text()').extract()
